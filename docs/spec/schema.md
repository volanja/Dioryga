...
**マイグレーション済みのテーブルは `docs/schema/` が正である。**tblsが実DBから生成しており、CIが陳腐化を検出する（設計書2章）。列の型・NULL可否・外部キー・インデックスは**そちらを見ること。**

| 章 | 状態 | 見る場所 |
|---|---|---|
| 1. 基盤 | 実装済み | **[`docs/schema/`](../schema/README.md)** |
| 2. カタログ | 実装済み | **[`docs/schema/`](../schema/README.md)** |
| 3. ハードウェア資産 | 実装済み | **[`docs/schema/`](../schema/README.md)** |
| 4. 物理設置・電源 | 実装済み | **[`docs/schema/`](../schema/README.md)** |
| 5. ネットワーク | 実装済み | **[`docs/schema/`](../schema/README.md)** |
| 7. QCD | 実装済み | **[`docs/schema/`](../schema/README.md)** |
| 6. ソフトウェア | 実装済み | **[`docs/schema/`](../schema/README.md)** |
| 8. 変更管理 | 実装済み | **[`docs/schema/`](../schema/README.md)** |
| 9. 監査・取込 | 実装済み | **[`docs/schema/`](../schema/README.md)** |
| 10. 機器認証情報 | **v2以降** | 本ファイル（暫定） |

**v1のスキーマはこれで一巡した。**残るのは10章（機器認証情報）だけで、v2以降の扱いである。

本ファイルの実装済みの節は、**マイグレーションを書く際の出発点として残している**（設計書のどの章に由来するかが分かるため）。実装との食い違いに気付いたら、直すのは本ファイルではなく設計リポジトリである。
...
- **`work_order_id`**：nullable。その変更を引き起こしたWORK_ORDER（11章）。**`WORK_ORDER` より先に作られたテーブルには外部キーが無い**——SQLiteが後から制約を足せないため（設計書24.3）。多態的参照と同じく `dioryga check`（24.5）で検出する
- **多態的参照**：`item_type`/`item_id`、`location_type`/`location_id` はDB外部キー制約を持てない。参照先の存在チェックはアプリケーション層で行う（4章、C-7）
- **物理名**：設計書のテーブル名をそのまま `snake_case` にする。例外は予約語と衝突する `USER` → **`app_user`** のみ（24.2.4）
- **自動マイグレーション**：サーバ起動時に未適用のマイグレーションを適用する。`database.auto_migrate = false` で無効化でき、その場合は `dioryga migrate` を明示実行する（24.1）
